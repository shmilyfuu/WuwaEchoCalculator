...
`main` 已切换到 Win32 + Direct2D 原生实现，当前发布版本为 `v1.3.1`。当前主线提交为 `4d04f5faf67e7211502efa69be316471dec89521`，窗口打开后应显示原生界面，客户区尺寸为 `1188 × 772 DIP`，深色背景上依次排列导入区、核对区、五张记录卡和右侧统计区。
...
`publish-v131.yml` 写死 `1.3.1`。后续版本建议读取 `VERSION`，并把 Release 标题、资产名称、标签和目录名统一从变量生成。新版本发布时只改一行版本号，Actions 日志会使用同一个值。
